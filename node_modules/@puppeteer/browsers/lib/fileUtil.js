 ...
 * @internal
 */
export async function extractZipWithYauzl(archivePath, folderPath) {
    const { default: yauzl } = await import(
    /* webpackIgnore: true */ 'yauzl').catch(() => {
 ...
 * @internal
 */
async function extractZipWithCli(archivePath, folderPath) {
    try {
...
async function extractZipEntry(zipFile, entry, folderPath) {
    ...
    const isDirectory = (unixMode & S_IFMT) === S_IFDIR ||
    ...
    const mode = unixMode === 0 ? (isDirectory ? 0o755 : 0o644) : unixMode & 0o777;
